...
import org.george.hybridcolumnar.column.sizeestimator.ColumnSizeEstimator;
import org.george.hybridcolumnar.column.sizeestimator.ColumnSizeEstimatorFactory;
...
public class ColumnEncoder {
	...
	public static<T extends Comparable<T>> Column<Comparable> encode(List<T> list) {
		long start = System.currentTimeMillis();
		ColumnAnalysisResult columnAnalysisResult = ColumnAnalyzer.analyze(list);
		long end = System.currentTimeMillis();
		System.out.println("total time: " + (end - start));
		System.out.println("" + columnAnalysisResult.getMaxNumber() + " "  + columnAnalysisResult.getMaxDelta());
		ColumnType bestEncoding = null;
		Long minSize = null;
		for (ColumnType columnType : ColumnType.values()) {
			if (!list.isEmpty() && !(list.get(0) instanceof Number) && (columnType == ColumnType.BIT_PACKING || columnType == ColumnType.DELTA)) {
				continue;
			}
			ColumnSizeEstimator estimator = ColumnSizeEstimatorFactory.createEstimator(columnType);
			long size = estimator.estimate(columnAnalysisResult);
		...
		Column<Comparable> column = ColumnFactory.createColumn(bestEncoding);
		for (T item : list) {
			column.add(item);
		}
		return column;
