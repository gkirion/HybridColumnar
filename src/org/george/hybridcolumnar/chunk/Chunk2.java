...
import org.george.hybridcolumnar.domain.Tuple4;

@SuppressWarnings("serial")
public class Chunk2<T1 extends Comparable<T1>, T2 extends Comparable<T2>>
		implements Iterable<Tuple4<T1, T2, Integer, Integer>>, Serializable {
	 ...
	 * row; ArrayList<Column<Integer>> cols = new ArrayList<>(); if
	 * (colNames.equals("*")) { for (String name : columns.keySet()) {
	 * cols.add(columns.get(name)); } } else { String[] names = colNames.split(",");
	 * for (String name : names) { cols.add(columns.get(name.trim())); } } for (int
	 * i = 0; i < id; i++) { row = new ArrayList<>(); for (Column<Integer> col :
	 * cols) { row.add(col.get(i).getFirst()); } rows.add(row); } return rows; }
	 * 
	 * public HashMap<Object, Integer> groupBy(String groupCols) { String[] colNames
	 * = groupCols.split(","); ArrayList<Column<Integer>> cols = new ArrayList<>();
	 * for (String colName : colNames) { cols.add(columns.get(colName)); }
	 * HashMap<Object, Integer> result = new HashMap<>(); ArrayList<Integer> row;
	 * for (int i = 0; i < id; i++) { row = new ArrayList<>(); for (Column<Integer>
	 * col : cols) { row.add(col.get(i).getFirst()); } if (result.get(row) == null)
	 * { result.put(row, 1); } else { result.put(row, result.get(row) + 1); } }
	 * return result; }
	 */
