...
public class ColumnDictionaryRle<E extends Comparable<E>> implements Column<E>, Serializable {
    ...
    public Tuple2<E, Integer> get(int i) {
        ...
        return new Tuple2<E, Integer>(dictionary.get(arrayList.get(key).getFirst()), arrayList.get(key).getThird() + arrayList.get(key).getSecond() - i);
    }
    ...
    public BitSet selectBetween(E from, E to) {
        ...
        for (int i = 0; i < n; i++) {
            tuple = arrayList.get(i);
            if (dictionary.get(tuple.getFirst()).compareTo(from) >= 0 && dictionary.get(tuple.getFirst()).compareTo(to) <= 0) {
                bitSet.set(tuple.getThird(), tuple.getThird() + tuple.getSecond());
            }
        }
        return bitSet;
    ...
    public Long sum(BitSet bitSet) {
        ...
        for (int i = 0; i < n; i++) {
            tuple = arrayList.get(i);
            sum += (Integer)dictionary.get(tuple.getFirst()) * bitSet.get(tuple.getThird(), tuple.getThird() + tuple.getSecond()).cardinality();
        }
        return sum;
    }
